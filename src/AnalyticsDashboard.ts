...
import { getSessions, getSessionDetails, Session, Message, ToolUse, ToolResult } from './historyProvider';
...
const FIELD_DIMENSIONS: FieldDimension[] = [
    { key: 'num_tool_calls',      label: 'Tool Calls',          description: 'Total tool invocations across the session' },
    ...
    { key: 'exploration_ratio',   label: 'Exploration Ratio',   description: 'Reads ÷ total tool calls (0=all action, 1=all reading)' },
    { key: 'commit_speed',        label: 'Commit Speed',        description: 'Normalized position of first edit (0=committed early, 1=committed late)' },
    { key: 'direction_changes',   label: 'Direction Changes',   description: 'Read→write mode switches (hesitation signal)' },
    { key: 'verification_effort', label: 'Verification Effort', description: 'Bash calls ÷ total tool calls (how much the agent checked its work)' },
    { key: 'error_rate',          label: 'Error Rate',          description: 'Fraction of tool results that reported errors' },
];

// ─── Behavioral Measurement (φ) ────────────────────────────────────────────

function measureSession(messages: Message[], sessionId: string, source: string, project: string, timestamp: number): SessionVector {
    ...
    for (const t of allToolUses) {
        const cur = ['Read', 'Glob', 'Grep'].includes(t.name) ? 'read'
                  : ['Edit', 'Write', 'MultiEdit'].includes(t.name) ? 'write'
                  : null;
    ...
    return {
        sessionId, source, project, timestamp,
        phi,
        statesReached, finalState: statesReached[statesReached.length - 1] ?? 'start',
        intentSequence, programString, actingCount, introspectingCount, actThinkCycles,
        outcome, errorRate, numErrors, numResults,
    };
}

function computeStates(toolUses: ToolUse[]): string[] {
    const states: string[] = ['start'];
    let hasRead = false, hasEdit = false, hasBash = false;
...
function countCycles(seq: string[]): number {
    let cycles = 0, inA = false;
    for (const s of seq) {
        if (s === 'A' && !inA) { cycles++; inA = true; }
...
function computeFieldMetrics(vectors: SessionVector[]): FieldMetricsResult {
    ...
    if (K === 0) {
        return { center: [], variance: [], width: 0, convergence: 0, separation: [], skew: [], successK: 0, failureK: 0 };
    }
    ...
    const outStd  = Math.sqrt(outcomes.reduce((a, b) => a + (b - outMean) ** 2, 0) / Math.max(K - 1, 1));
    ...
    for (let j = 0; j < d; j++) {
        const sucMean = successes.length > 0
            ? successes.reduce((a, v) => a + v.phi[j], 0) / successes.length : 0;
        const failMean = failures.length > 0
            ? failures.reduce((a, v) => a + v.phi[j], 0) / failures.length : 0;
        separation.push(sucMean - failMean);
    }

    const skew: number[] = [];
    for (let j = 0; j < d; j++) {
        skew.push(pearsonCorrelation(outcomes, vectors.map(v => v.phi[j])));
    }

    return { center, variance, width, convergence, separation, skew, successK: successes.length, failureK: failures.length };
}
...
async function runPythonAft(vectors: SessionVector[], venvPath: string): Promise<Record<string, unknown>> {
...
`;
    ...
    return new Promise(resolve => {
        const proc = cp.execFile(pythonBin, [scriptFile, dataFile, outputFile], { timeout: 30000 }, (err, _stdout, stderr) => {
            try {
...
export class AnalyticsDashboard {
    ...
    public static createOrShow(context: vscode.ExtensionContext) {
        ...
        panel.onDidDispose(() => { AnalyticsDashboard._instance = undefined; });
    }
    ...
    private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext) {
        ...
        this._panel.webview.onDidReceiveMessage(
            async (msg: { type: string; venvPath?: string; scriptsPath?: string; maxSessions?: number }) => {
                if (msg.type === 'ready' || msg.type === 'refresh') {
                    await this._sendAnalytics();
                } else if (msg.type === 'saveSettings') {
                    const cfg = vscode.workspace.getConfiguration('aisx.analytics');
                    if (msg.venvPath !== undefined)    await cfg.update('pythonVenvPath',   msg.venvPath,   vscode.ConfigurationTarget.Global);
                    if (msg.scriptsPath !== undefined) await cfg.update('customScriptsPath', msg.scriptsPath, vscode.ConfigurationTarget.Global);
                    if (msg.maxSessions !== undefined) await cfg.update('maxSessions',      msg.maxSessions, vscode.ConfigurationTarget.Global);
                    await this._sendAnalytics();
    ...
    private async _testPython(venvPath: string): Promise<{ ok: boolean; message: string }> {
        ...
        const pythonBin = process.platform === 'win32'
            ? path.join(venvPath, 'Scripts', 'python.exe')
            : path.join(venvPath, 'bin', 'python');
        if (!fs.existsSync(pythonBin)) return { ok: false, message: `Python not found at ${pythonBin}` };

        return new Promise(resolve => {
            cp.execFile(pythonBin, ['-c', 'import agent_fields; print("agent_fields OK")'], { timeout: 10000 }, (err, stdout, stderr) => {
    ...
    private async _sendAnalytics() {
        ...
        try {
            ...
            for (const sess of limited) {
                try {
                    ...
                    if (!detail.error && detail.messages) {
                        vectors.push(measureSession(detail.messages, sess.id, sess.source, sess.project, sess.timestamp));
                    }
                } catch {
            ...
            for (const v of vectors) projCounts.set(v.project, (projCounts.get(v.project) ?? 0) + 1);
